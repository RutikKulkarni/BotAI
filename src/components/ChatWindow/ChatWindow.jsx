...
const ChatWindow = ({ conversationId, onAskQuestion }) => {
  const [messages, setMessages] = useState([]);
  const [selectedQuestion, setSelectedQuestion] = useState(null);
  const [hoverIndex, setHoverIndex] = useState(-1); // Index of the message user is hovering over
  const messageContainerRef = useRef(null);
  ...
  const handleLikeDislike = (index, like) => {
    ...
    console.log(`Message at index ${index} ${like ? 'liked' : 'disliked'}`);
  };
                    ...
                    <div className={styles.messageContainer}>
                      <div className={styles.messageContent}>
                        ...
                        <div className={styles.messageText}>
                          ...
                          <div>{message.text}</div>
                          {(message.sender === "AI" && (index === messages.length - 1 || index === hoverIndex)) ? (
